...
//    manager.operationQueue.maxConcurrentOperationCount = 1;
    ...
    NSString *range = [NSString stringWithFormat:@"bytes=%zd-",[dict[@"completedUnitCount"] integerValue]];
    
    __block NSDictionary *tempDict = dict;
    
    
    NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:[NSURL URLWithString:dict[@"url"]]];
    
    [request setValue:range forHTTPHeaderField:@"Range"];
    
    
    
    return  [manager dataTaskWithRequest:request uploadProgress:^(NSProgress * _Nonnull uploadProgress) {
