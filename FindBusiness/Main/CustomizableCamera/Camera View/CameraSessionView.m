...
-(void)composeInterface {
    ...
    if ( UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad )
    {
        ...
        topBarSize        = CGSizeMake(70, [[UIScreen mainScreen] bounds].size.width);
    ...
    if (_topBarView) {
        ...
        _topBarView.frame  = (CGRect){SCREEN_WIDTH-topBarSize.width,0, topBarSize};
...
- (void)onTapDismissButton {
    [UIView animateWithDuration:0.3 animations:^{
        ...
        if ([self.delegate respondsToSelector:@selector(dismissTapAction)]) {
            [self.delegate dismissTapAction];
        }
